...
class ArchivePastExcursions extends Command
{
    protected $signature = 'excursions:archive-past {--dry-run : Только показать, без изменений}';

    protected $description = 'Архивировать все прошедшие экскурсии (дата/время меньше текущего момента)';

    public function handle(): int
    {
        $dryRun = $this->option('dry-run');
        
        $this->info('Поиск прошедших экскурсий...');
        ...
        if ($pastExcursions->isEmpty()) {
            $this->info('Нет прошедших экскурсий для архивации.');
            return self::SUCCESS;
        }
        ...
        if ($dryRun) {
            foreach ($pastExcursions->take(30) as $excursion) {
            ...
            if ($pastExcursions->count() > 30) {
                $this->line("... и еще " . ($pastExcursions->count() - 30) . " экскурсий");
            }
            return self::SUCCESS;
        }
        
        $archived = 0;
        foreach ($pastExcursions->chunk(50) as $chunk) {
        ...
        $this->info("Архивировано {$archived} прошедших экскурсий");
        return self::SUCCESS;
    }
        ...
        DB::transaction(function () use ($excursion) {
            ...
            foreach ($excursion->bookings as $booking) {
                ...
                ArchivedBooking::create([
                    ...
                    'archived_reason' => 'Past excursion',
                    'archived_at' => now(),
                ]);
